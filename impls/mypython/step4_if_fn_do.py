...
from m_types import Boolean, Form, List_, Map, Nil, Symbol, Vector
from env import Env
        ...
        case List_():
            ...
            match ast:
                ...
                case List_([Symbol("do"), *operand]):
                    for expr in operand:  # type: ignore
                        out = eval_(expr, env)
                ...
                case List_([Symbol("fn*"), List_() | Vector() as binds, contents]):

                    def closure(*exprs):
                        c_env = Env(env, binds, exprs)
                        return eval_(contents, c_env)
